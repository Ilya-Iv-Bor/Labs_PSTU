...
class FunctionData { //Данные функции
public:
    double x1 = 0;
    double x2 = 0;
    double x3 = 0;
...
class EquationData { //Данные уровнения 
public:
    FunctionData func;
    double left = 0;
    double right = 0;
...
double solve(EquationData data) { //Выыбор решений
    double x;
    switch (data.method) {
...
double equation(FunctionData func, double x) {//Вид уравнения
...
double findNewton(EquationData data) { //Метод Ньютона
    double f, df, x0;
    double x = data.right;
    int iter = 0;
...
double findDiv(EquationData data) { //Метод половинного деления
    FunctionData func = data.func;
...
double findIter(EquationData data) { //Метод итераций
    FunctionData func = data.func;
    double x0;
